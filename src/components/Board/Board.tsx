...
const Board: React.FC = () => {
  ...
  const currentCard = useMemo(
    () => todoList.find((todo) => todo.id === currentCardId),
          ...
          <div>
            <Columns $columnsCount={columns.length}>
              ...
              {isEdit
                ...
                : (
                  <CentredFlex>
                    <NewColumn type="text" onChange={handleChange} value={columnTitle} autoFocus />
                    <Flex>
...
const Columns = styled.div<{ $columnsCount: number }>`
  display: flex;
  width: ${({ $columnsCount }) => `${320 * ($columnsCount + 1)}px`};
